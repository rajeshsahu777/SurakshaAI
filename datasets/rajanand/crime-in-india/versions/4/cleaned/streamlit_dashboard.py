...
all_districts = sorted(df[district_col].dropna().unique())
selected_district = st.sidebar.selectbox("Focus District", ["All Districts"] + list(all_districts))

df_filtered = df[(df[year_col] >= selected_years[0]) & (df[year_col] <= selected_years[1])]
if selected_district != "All Districts":
    df_filtered = df_filtered[df_filtered[district_col] == selected_district]
...
if page == "📊 Overview Dashboard":
    ...
    total_crimes  = int(df_filtered['TOTAL_CRIMES'].sum()) if 'TOTAL_CRIMES' in df_filtered.columns else 0
    high_risk_cnt = int((df_filtered['RISK_LABEL'] == 2).sum()) if 'RISK_LABEL' in df_filtered.columns else 0
    district_cnt  = df_filtered[district_col].nunique()
    ...
    with col_l:
        ...
        if 'TOTAL_CRIMES' in df_filtered.columns:
            yearly = df_filtered.groupby(year_col)['TOTAL_CRIMES'].sum()
    ...
    with col_r:
        ...
        if 'TOTAL_CRIMES' in df_filtered.columns:
            top_d = df_filtered.groupby(district_col)['TOTAL_CRIMES'].sum().nlargest(10)
            fig, ax = dark_fig(7, 3.5)
    ...
    with col_l2:
        ...
        if 'RISK_LABEL' in df_filtered.columns:
            rc = df_filtered['RISK_LABEL'].map({0:'LOW',1:'MEDIUM',2:'HIGH'}).value_counts()
            fig, ax = plt.subplots(figsize=(5, 3.5))
            fig.patch.set_facecolor('#161b22')
            ax.set_facecolor('#161b22')
            clrs_pie = [{'HIGH':'#f85149','MEDIUM':'#e3b341','LOW':'#3fb950'}.get(k,'#58a6ff') for k in rc.index]
            wedges, texts, autotexts = ax.pie(rc.values, labels=rc.index, colors=clrs_pie,
                                               autopct='%1.1f%%', textprops={'color':'#e6edf3','fontsize':10})
            for at in autotexts:
    ...
    with col_r2:
        ...
        if crime_cols:
            top_crimes = df_filtered[crime_cols].sum().nlargest(8)
            fig, ax = dark_fig(7, 3.5)
...
# ============================================================
    ...
    if os.path.exists(os.path.join(OUTPUT_PATH, 'hotspot_clusters.png')):
        st.image(os.path.join(OUTPUT_PATH, 'hotspot_clusters.png'), use_column_width=True)
...
# ============================================================
    ...
    with col1:
        ...
        if st.button("🔍 Predict Risk", use_container_width=True):
            ...
            if row.empty:
                ...
                rv = int(row['RISK_LABEL'].values[0])
                risk_map = {
                    0: ('🟢 LOW RISK',    '#3fb950'),
                    1: ('🟡 MEDIUM RISK', '#e3b341'),
                    2: ('🔴 HIGH RISK',   '#f85149')
                }
                label, color = risk_map.get(rv, ('UNKNOWN', '#8b949e'))
    ...
    if os.path.exists(os.path.join(OUTPUT_PATH, 'xgb_results.png')):
        ...
        st.image(os.path.join(OUTPUT_PATH, 'xgb_results.png'), use_column_width=True)
...
# ============================================================
    ...
    if 'forecast' in data:
        df_fc = data['forecast']
        fc_district = st.selectbox("Select District", sorted(df_fc[district_col].unique()))
        df_d = df_fc[df_fc[district_col] == fc_district].sort_values('year')
        historical = df_d[df_d['is_forecast'] == False]
        forecast   = df_d[df_d['is_forecast'] == True]

        fig, ax = dark_fig(10, 4)
        if not historical.empty:
            ax.plot(historical['year'], historical['predicted_crimes'],
                    color='#58a6ff', linewidth=2.5, marker='o', markersize=5, label='Historical')
        if not forecast.empty:
            ax.plot(forecast['year'], forecast['predicted_crimes'],
                    color='#f85149', linewidth=2.5, linestyle='--', marker='s', markersize=5, label='Forecast')
            if 'lower_bound' in forecast.columns:
                ax.fill_between(forecast['year'], forecast['lower_bound'], forecast['upper_bound'],
                                alpha=0.2, color='#f85149', label='Confidence Band')
        ax.set_title(f'Crime Forecast — {fc_district}', color='#e6edf3', fontsize=13)
        ax.set_xlabel('Year', color='#8b949e'); ax.set_ylabel('Predicted Crimes', color='#8b949e')
        ax.legend(facecolor='#161b22', edgecolor='#30363d', labelcolor='#e6edf3')
        st.pyplot(fig, use_container_width=True)
        plt.close()

        if 'trend_direction' in df_fc.columns:
            st.markdown("### 🚨 Emerging Threat Districts")
            threats = df_fc[(df_fc['is_forecast'] == True) & (df_fc['trend_direction'] == '↑ INCREASING')
                           ][[district_col, 'year', 'predicted_crimes', 'trend_direction']].drop_duplicates(district_col)
        ...
        if os.path.exists(os.path.join(OUTPUT_PATH, 'forecast_top5_districts.png')):
            st.image(os.path.join(OUTPUT_PATH, 'forecast_top5_districts.png'), use_column_width=True)
...
# ============================================================
    ...
    if 'anomaly' in data:
        ...
        if os.path.exists(os.path.join(OUTPUT_PATH, 'anomaly_results.png')):
            ...
            st.image(os.path.join(OUTPUT_PATH, 'anomaly_results.png'), use_column_width=True)
    else:
